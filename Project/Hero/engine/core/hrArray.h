...
#ifndef __HERO_ARRAY_H_INCLUDED__
#define __HERO_ARRAY_H_INCLUDED__
...
*/
template <class T, typename TAlloc = hrAllocator<T> >
class array
{
...
public:
	...
	*/
	void reallocate(uint32 new_size, bool canShrink=true)
	{
		...
		// copy old data
		int32 end = used < new_size ? used : new_size;
	...
	void insert(const T& element, uint32 index=0)
	{
	...
	application. */
	void set_pointer(T* newPointer, uint32 size, bool _is_sorted=false, bool _free_when_destroyed=true)
	{
		clear();
		data = newPointer;
		allocated = size;
	...
	int32 binary_search(const T& element)
	{
		sort();
	...
	\return Position of the searched element if it was found,
	otherwise -1 is returned. */
	int32 binary_search(const T& element) const
	{
		if (is_sorted)
			return binary_search(element, 0, used-1);
		else
	...
	is returned. */
	int32 binary_search(const T& element, int32 left, int32 right) const
	{
		if (!used)
			return -1;
	...
	otherwise -1 is returned. */
	int32 binary_search_multi(const T& element, int32 &last)
	{
		sort();
		int32 index = binary_search(element, 0, used-1);
		if ( index < 0 )
			return index;
	...
	\return Position of the searched element if it was found, otherwise -1
	is returned. */
	int32 linear_search(const T& element) const
	{
		for (uint32 i=0; i<used; ++i)
			if (element == data[i])
				return (int32)i;
	...
	\return Position of the searched element if it was found, otherwise -1
	is returned. */
	int32 linear_reverse_search(const T& element) const
	{
