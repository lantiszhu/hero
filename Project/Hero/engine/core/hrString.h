...
// This file is part of the "Irrlicht Engine" and the "hrXML" project.
// For conditions of distribution and use, see copyright notice in hrlicht.h and hrXML.h

#ifndef __HERO_STRING_H_INCLUDED__
#define __HERO_STRING_H_INCLUDED__

#include "hrTypes.h"
#include "hrAllocator.h"
#include "hrMath.h"
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
static inline uint32 locale_lower ( uint32 x )
{
...
static inline uint32 locale_upper ( uint32 x )
{
	switch ( locale_current )
	{
		case IRR_LOCALE_GERMAN:
...
template <typename T, typename TAlloc = hrAllocator<T> >
class string
{
public:
	...
	template <class B>
	string(const B* const c, uint32 length)
	: array(0), allocated(0), used(0)
	{
		if (!c)
		{
	...
	{
		for (uint32 i=0; array[i] && other.array[i]; ++i)
	...
	bool operator<(const string<T,TAlloc>& other) const
	{
		for (uint32 i=0; array[i] && other.array[i]; ++i)
		{
			const int32 diff = array[i] - other.array[i];
			if (diff)
				return (diff < 0);
		}
	...
	{
		for(uint32 i=0; array[i] && other[i]; ++i)
	...
	\return True if the strings are equal ignoring case. */
	bool equals_substring_ignore_case(const string<T,TAlloc>&other, const int32 sourcePos = 0 ) const
	{
	...
	bool lower_ignore_case(const string<T,TAlloc>& other) const
	{
		for(uint32 i=0; array[i] && other.array[i]; ++i)
		{
			int32 diff = (int32) locale_lower ( array[i] ) - (int32) locale_lower ( other.array[i] );
			if ( diff )
				return diff < 0;
		}
	...
	bool equalsn(const string<T,TAlloc>& other, uint32 n) const
	{
		uint32 i;
	...
	string<T,TAlloc>& append(const T* const other, uint32 length=0xffffffff)
	{
		if (!other)
			return *this;
	...
	or -1 if not found. */
	int32 findFirstChar(const T* const c, uint32 count=1) const
	{
		if (!c || !count)
			return -1;
	...
	or -1 if not found. */
	template <class B>
	int32 findFirstCharNotInList(const B* const c, uint32 count=1) const
	{
		if (!c || !count)
			return -1;
	...
	or -1 if not found. */
	template <class B>
	int32 findLastCharNotInList(const B* const c, uint32 count=1) const
	{
		if (!c || !count)
			return -1;
	...
	{
		start = core::clamp ( start < 0 ? (int32)(used) - 2 : start, 0, (int32)(used) - 2 );
	...
	\return Position where one of the characters has been found,
	or -1 if not found. */
	int32 findLastChar(const T* const c, uint32 count=1) const
	{
		if (!c || !count)
			return -1;
	...
	string<T> subString(uint32 begin, int32 length, bool make_lower = false ) const
	{
	...
	string<T,TAlloc>& replace(const string<T,TAlloc>& toReplace, const string<T,TAlloc>& replaceWith)
	{
		...
		const uint32 other_size = toReplace.size();
		const uint32 replace_size = replaceWith.size();

		// Determine the delta.  The algorithm will change depending on the delta.
		int32 delta = replace_size - other_size;
		...
		if (delta == 0)
		{
			...
			{
				for (uint32 i = 0; i < replace_size; ++i)
		...
		{
			uint32 i = 0;
			for (uint32 pos = 0; pos < used; ++i, ++pos)
			{
	...
	string<T,TAlloc>& removeChars(const string<T,TAlloc> & characters)
	{
		...
		for (uint32 i=0; i<used-1; ++i)
		{
			...
			for (uint32 j=0; j<characters.size(); ++j)
			{
	...
	string<T,TAlloc>& erase(uint32 index)
	{
...
private:
	...
	void reallocate(uint32 new_size)
	{
		...
		uint32 amount = used < new_size ? used : new_size;
