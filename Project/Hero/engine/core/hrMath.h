...
#ifndef __HERO_MATH_H_INCLUDED__
#define __HERO_MATH_H_INCLUDED__

#include "hrCompileConfig.h"
#include "hrTypes.h"
#include <math.h>
#include <float.h>
...
#include <limits.h> // For INT_MAX / UINT_MAX
	...
	#define sqrtf(X) (hero::f32)sqrt((hero::f64)(X))
	#define sinf(X) (hero::f32)sin((hero::f64)(X))
	#define cosf(X) (hero::f32)cos((hero::f64)(X))
	#define asinf(X) (hero::f32)asin((hero::f64)(X))
	#define acosf(X) (hero::f32)acos((hero::f64)(X))
	#define atan2f(X,Y) (hero::f32)atan2((hero::f64)(X),(hero::f64)(Y))
	#define ceilf(X) (hero::f32)ceil((hero::f64)(X))
	#define floorf(X) (hero::f32)floor((hero::f64)(X))
	#define powf(X,Y) (hero::f32)pow((hero::f64)(X),(hero::f64)(Y))
	#define fmodf(X,Y) (hero::f32)fmod((hero::f64)(X),(hero::f64)(Y))
	#define fabsf(X) (hero::f32)fabs((hero::f64)(X))
	#define logf(X) (hero::f32)log((hero::f64)(X))
#endif
...
namespace core
{
	...
	const int32 ROUNDING_ERROR_S32 = 0;
#ifdef __HERO_HAS_S64
...
#undef PI64
	...
	*/
	...
	inline bool equals(const int32 a, const int32 b, const int32 tolerance = ROUNDING_ERROR_S32)
	{
		return (a + tolerance >= b) && (a - tolerance <= b);
	}

	//! returns if a equals b, taking an explicit rounding tolerance into account
	inline bool equals(const uint32 a, const uint32 b, const int32 tolerance = ROUNDING_ERROR_S32)
	{
		return (a + tolerance >= b) && (a - tolerance <= b);
	}
	...
	//! returns if a equals zero, taking rounding errors into account
	inline bool iszero(const int32 a, const int32 tolerance = 0)
	{
		return ( a & 0x7ffffff ) <= tolerance;
	}

	//! returns if a equals zero, taking rounding errors into account
	inline bool iszero(const uint32 a, const uint32 tolerance = 0)
	{
		return a <= tolerance;
	}
	...
	inline int32 int32_clamp (int32 value, int32 low, int32 high)
	{
	...
	*/
	...
	typedef union { uint32 u; int32 s; f32 f; } inttofloat;
...
#else
	inline uint32 IR(f32 x) {inttofloat tmp; tmp.f=x; return tmp.u;}
#endif
	...
	#define FR(x)                           ((f32&)(x))
#else
	inline f32 FR(uint32 x) {inttofloat tmp; tmp.u=x; return tmp.f;}
	inline f32 FR(int32 x) {inttofloat tmp; tmp.s=x; return tmp.f;}
#endif
	...
	REALINLINE uint32 if_c_a_else_b ( const c8 condition, const uint32 a, const uint32 b )
	{
	...
	//! conditional set based on mask and arithmetic shift
	REALINLINE uint32 if_c_a_else_b ( const int32 condition, const uint32 a, const uint32 b )
	{
	...
	//! conditional set based on mask and arithmetic shift
	REALINLINE uint32 if_c_a_else_0 ( const int32 condition, const uint32 a )
	{
		return ( -condition >> 31 ) & a;
	}
#endif
	...
	*/
	REALINLINE void setbit_cond ( uint32 &state, int32 condition, uint32 mask )
	{
...
#  warn clearFPUException not supported.
	...
	// calculate: sqrt ( x )
	REALINLINE int32 squareroot(const int32 f)
	{
		return static_cast<int32>(squareroot(static_cast<f32>(f)));
	}
...
*/
	...
	// calculate: 1 / sqrtf( x )
	REALINLINE int32 reciprocal_squareroot(const int32 x)
	{
		return static_cast<int32>(reciprocal_squareroot(static_cast<f32>(x)));
	}
...
*/
	...
	REALINLINE int32 floor32(f32 x)
	{
#ifdef IRRLICHT_FAST_MATH
		const f32 h = 0.5f;
...
#  warn IRRLICHT_FAST_MATH not supported.
	...
	REALINLINE int32 round32(f32 x)
	{
#if defined(IRRLICHT_FAST_MATH)
		int32 t;
